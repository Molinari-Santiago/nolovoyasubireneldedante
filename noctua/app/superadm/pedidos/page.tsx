...
import { Plus, Trash2, Layout, Utensils, X } from 'lucide-react';
...
export default function SuperAdmPedidosPage() {
  ...
  const [tab, setTab] = useState<'categorias' | 'platos'>('categorias');
  const [deleteId, setDeleteId] = useState<string | null>(null);
  const [deleteProductoId, setDeleteProductoId] = useState<string | null>(null);
  const [adding, setAdding] = useState(false);
  const [editing, setEditing] = useState<'categoria' | 'producto' | null>(null);
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-[#676b67]">Gestiona las categorías y platos del menú</p>
        </div>
      </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-[#252525]">
                {productos.map((prod) => (
                  <tr key={prod.id} className="hover:bg-[#101010]">
                    <td className="py-4 pl-4 text-white font-medium">{prod.nombre}</td>
                    <td className="py-4 text-[#676b67]">
                      {categorias.find(c => c.id === prod.categoria_id)?.nombre || 'Sin categoría'}
                    </td>
                    <td className="py-4 text-white font-mono">${prod.precio.toFixed(2)}</td>
                    <td className="py-4">
                      <span className={`px-2 py-1 rounded text-xs font-bold ${
                        prod.disponible ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'
                      }`}>
                    ...
                    <td className="py-4 pr-4 text-right">
                      ...
                      <button
                        onClick={() => setDeleteProductoId(prod.id)}
                        className="text-red-400 hover:text-red-300"
                      >
                        Eliminar
                      </button>
      ...
      {editing && editForm && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-end z-50">
          <div className="w-full max-w-md h-full bg-[#0d0d0d] border-l border-[#252525] p-8 overflow-y-auto">
